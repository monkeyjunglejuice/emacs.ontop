...
- Easy to change leader keymap(s)
  + agnostic leader + local leader for Evil, God Mode, Helix, Meow -- implemented with plain keymaps and without any 3rd party packages like General et. al.
  + default pre-configured leader keymap
...
- You cam customize EON without touching code if you prefer by just using Emacs' Customization UI. That comes with some perks like type checked variables (use `setopt` instead of plain `setq`).
