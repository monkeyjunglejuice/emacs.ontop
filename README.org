...
0. If you disable them all, you have bare bones, stock Emacs.
...
2. With =eon= and =eon-base= modules enabled, you'll get extended comfort.
...
- *3.* Uncomment your desired modules in =~/.emacs.ontop/eon-setup-modules.el=, then restart Emacs.
...
* Get comfortable
Look into the module files in order to learn about the commands and key bindings -- all documentation lives there. If you want to change settings, re-define these settings within your =init.el= (recommended) or =eon-setup-personal.el=.
...
| - "Fast" startup (masked), lags often at runtime             | - Slower startup, snappy at runtime                  |
| - Strongly opinionated                                       | - Provides options, bend it your way                 |
| - Focus on VIM-keybindings (Evil), otherwise under-developed | - Agnostic: God-mode, Evil, stock Emacs, Helix, Meow |
...
| - Backwards-compatible to Emacs 26                           | - Minimum required Emacs version 30.1                |
...
| - Many Doomishms!                                            | - Try hard to re-use built-in facilities             |
...
| Old and established packages (Helm, ...) | Newer packages (Consult, Vertico, ...) |
