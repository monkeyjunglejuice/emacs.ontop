...
*eon-lang-commonlisp-sly*: Complete Common Lisp setup with [[https://github.com/joaotavora/sly][Sly]]. Just install SBCL with your package manager and then [[https://www.quicklisp.org/beta/][Quicklisp]]. The setup covers most needs, but is pre-configured for SBCL right now.

*eon-lang-commonlisp-slime*: Alternative Common Lisp setup with [[https://github.com/slime/slime][Slime]] that Mirrors the Sly setup.
...
** Emacs for OCaml ★★★★☆
*eon-lang-ocaml-tuareg*: OCaml setup in Emacs based on Tuareg and Merlin. Not LSP based right now, as using pure Merlin feels better integrated. Completion in Utop works, with completion candidates provided by Merlin.

*eon-lang-ocaml-neocaml*: Alternative OCaml setup based on NeoCaml (WIP).
...
** Emacs for Janet  ★★★☆☆
*eon-lang-janet*: Almost complete Janet config for Emacs with Treesitter, REPL, LSP support and Org mode integration. The tooling itself is not that mature yet, as Janet is a pretty young language. The required packages are installed
from Github repos.
