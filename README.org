...
EON is a new Emacs distribution in the making, built on top of the zero-deps [[https://github.com/monkeyjunglejuice/emacs.onboard][Emacs ONboard starter kit]].

* Features

- Choose your keybindings: Evil (Vim), God Mode, Helix, Meow or Emacs. EON is not tied to a single editing model.

- Easy to change leader keymap(s)
  + agnostic leader + local leader for God Mode, Helix, Meow -- implemented with plain keymaps and without any 3rd party packages like General et. al.
  + default pre-configured leader keymap(s)
  + tabula rasa leader keymap for you to populate -- if you don't like the default
  + or just bring your own keymap(s) and simply bind it to the leader/local-leader machinery.

- You cam customize EON without touching code if you prefer by just using Emacs' Customization UI. That comes with some perks like type checks (use ~setopt~ instead of plain ~setq~).

- Fully modular with dependency/conflict management (e.g. Smartparens vs. Paredit)
  + separation of built-in, contrib and user-defined modules
  + override built-in modules with contrib- or personal modules
  + modules follow Emacs package conventions (incl. metadata)
  + contrib modules are standard Emacs (meta-)packages and can be published on Melpa, as Git repos, or just as a bunch of Elisp files (WIP)

- It's all just libraries: EON tries to integrate into your pre-existing Emacs config rather than to replace it. But that also means EON doesn't handle stuff that works only in =early-init.el=.

* Modular Architecture

EON is supposed to be completely modular with strictly separated concerns. There are 4 stages:

1. If you disable all modules in your [[https://github.com/monkeyjunglejuice/emacs.ontop/blob/main/eon-setup-modules.el][eon-setup-modules]], you'll get bare bones, stock Emacs.

2. With just the =eon= module enabled but nothing else, only built-in Emacs packages are configured -- without any 3rd-party dependencies.

3. With both =eon= and =eon-base= modules enabled, you'll get extended comfort provided by some 3rd-party packages. This is how your fresh installation starts out.
...
* LLM / Agent integration (WIP)

EON comes with the the amazing Gptel ... well, "framework"? Because it follows the Emacs philosophy of deep, fine-grained integration. Supports local LLMs too via Ollama (Llama.cpp planned).

* Use Cases

** 1. Programming -- Conventional, Interactive and Notebook-style
...
*eon-lang-commonlisp-slime*: Complete Common Lisp setup with [[https://github.com/slime/slime][Slime]]. Just install SBCL with your package manager and then [[https://www.quicklisp.org/beta/][Quicklisp]]. The setup covers most needs, but is pre-configured for SBCL right now.

*eon-lang-commonlisp-sly*: Alternative Common Lisp setup with [[https://github.com/joaotavora/sly][Sly]] that Mirrors the Slime setup.

*** Emacs for Elixir ★★★★☆
*eon-lang-elixir*: Elixir setup comes with the new "Expert" language server pre-configured; Tree-sitter, Mix integration and a REPL. [[https://github.com/J3RN/inf-elixir/issues/20][Autocompletion in the REPL doesn't work]] yet, as it hasn't been implemented upstream.
...
*** Emacs for OCaml ★★★★☆
*eon-lang-ocaml-tuareg*: OCaml setup in Emacs based on Tuareg and Merlin. Not LSP based, as using Merlin pure feels better integrated. Completion in Utop works, with completion candidates provided by Merlin.

*eon-lang-ocaml-neocaml*: (WIP) Alternative OCaml setup based on NeoCaml.
...
*** Emacs for Scheme ★★★★☆
*eon-lang-scheme*: The Emacs Scheme setup supports 10 major Scheme implementations via Geiser and comes with SRFI-browser and structural editing. The Scheme implementations have either more or less comprehensive tooling, hence the user experience varies. Tested on Debian Linux, Guix and macOS with GNU Guile (most interactive experience) and Chicken scheme (rather bare-bones).
...
*** Emacs for Janet  ★★★☆☆
*eon-lang-janet*: Almost complete Janet config for Emacs with Treesitter, REPL, LSP support and Org mode integration. The tooling itself is not that mature yet, as Janet is a pretty young language. The required packages are installed from Github repos rather than Melpa/Elpa.

*** Emacs for LFE (Lisp Flavoured Erlang) ★★★☆☆
*eon-lang-lfe*: Basic LFE setup with REPL. The LFE REPL Emacs frontend (inferior-lfe) could benefit from some polishing and refactoring, maybe I'll tackle in case I'll do something with LFE.
...
*** Emacs for Unison ★★☆☆☆
*eon-lang-unison*: Includes the treesitter-enabled Unison mode and LSP config for Eglot that uses UCM headless.
...
- *3.* Restart Emacs. Built-in Emacs packages are configured and a few basic 3rd-party packages installed, defined in =eon-base.el=.
...
| Doom Emacs                                             | EON                                                        |
|--------------------------------------------------------+------------------------------------------------------------|
| - Thousands of users                                   | - Few users                                                |
| - Throw away your own Emacs config                     | - Non-invasive; Eon integrates as a library                |
| - "Fast" startup (masked), lags at runtime             | - Slower startup (not optimized yet), snappy at runtime    |
| - Focus on VIM-keybindings (Evil)                      | - Agnostic: God-mode, Evil, stock Emacs, Meow, Helix (WIP) |
...
| - User directory for personal config                   | - User directory for personal config                       |
...
| - Partly backwards-compatible to Emacs 28.1            | - Minimum required Emacs version 30.1                      |
| - Uses Straigth.el package manager                     | - Uses built-in Package.el, others possible (WIP)          |
...
| Spacemacs                                            | EON                                                |
|------------------------------------------------------+----------------------------------------------------|
| Community-driven, many users                         | Single-person project, few users                   |
