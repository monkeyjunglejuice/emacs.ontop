...
Emacs ONtop is a new Emacs distribution in the making, built on top of the [[https://github.com/monkeyjunglejuice/emacs.onboard][Emacs ONboard starterkit]].

- /Under heavy development right now, things may break often in master. For a more stable experience, use the latest tagged release./
...
1. If you disable all modules in your =eon-setup-modules.el=, you'll get bare bones, stock Emacs.

2. With just the =eon.el= module enabled but nothing else, only built-in Emacs packages are set up and configured, without any 3rd-party dependencies.

3. With =eon= and =eon-base= modules enabled, you'll get extended comfort provided by some 3rd-pard packages.

4. Everything else builds on top of the stages 1, 2 and 3: AI/LLM integration, programming languages, etc. (have a look at =eon-setup-modules= for the full list).

* Use Case: Programming

Programming language support is rather limited at that time; it covers merely languages I personally like & use -- Lisp, functional, interactive. So there's no support for Python, Rust, JS or Go right now. However, if you'd like to use any of the languages in Emacs that are listed below, give it a go.
...
** Emacs for OCaml ★★★★☆
*eon-lang-ocaml*: OCaml setup in Emacs based on Tuareg and Merlin. Not LSP based right now, as using pure Merlin provides more features. Completion in Utop is working, with completion candidates provided by Merlin.
...
** Emacs for Elixir ★★★★☆
*eon-lang-elixir*: Elixir setup comes with the language server pre-configured (currently Elixir-ls, until the new one is available); Tree-sitter, Mix integration and a REPL. [[https://github.com/J3RN/inf-elixir/issues/20][Autocompletion doesn't work in the REPL]] yet, unfortunately. "Jump-to-definition" doesn't work either, but that limitation comes from the language server.

** Emacs for Erlang ★★☆☆☆
*eon-lang-erlang*: Basic Erlang setup with ELP (Whatsapp's language server), Erlang shell, Tree-sitter and Org-mode babel for notebook-style literate programming. 2 stars only because docs lookup doesn't work yet; I guess that requires some wrangling with man pages. Doesn't include things like Edts or Distel, as they rely dependencies at the UI level. Packages come from Melpa right now, not using the Emacs packages distributed with the Erlang/OTP installation. In the future, there may be a choice which package source to use.
...
** Emacs for Gleam ★★☆☆☆
*eon-lang-gleam*: Basic setup for the Gleam language with LSP and Treesitter support. 2 stars only because there's no interactive programming -- Gleam doesn't come with a REPL unfortunately.
...
** Emacs for Racket ★★★★☆
*eon-lang-racket*: The Racket setup based on [[https://github.com/greghendershott/racket-mode][Racket-mode]] provides more interactivity than DrRacket, and the new "racket-hash-lang-mode" supports arbitrary syntax for DSLs in Emacs. However, the syntax highlighting could be configured more in-depth. As I strongly prefer Guile instead of Racket, this won't change in the forseeable future though.

** Emacs for Lua ★★☆☆☆
*eon-lang-lua*: Basic Lua setup with REPL and lua-language-server. Rudimentary setup that suits my needs of editing existing Lua code -- the available features don't come close to the Lua IDE [[https://studio.zerobrane.com/][ZeroBrane Studio]], [[https://github.com/orbitalquark/textadept][TextAdept]] or Neovim. If you are serious about Lua hacking on a reguar basis, check out [[https://github.com/orbitalquark/textadept][TextAdept]] though -- it's higly under-rated.

** Emacs for Web Development ★☆☆☆☆
*eon-lang-webdev*: Just basic HTML, CSS and JavaScript editing and linting (linters have to be installed separately). Highly under-developed, since I don't do web stuff usually besides some CSS styling.

* Use Case: Prose / Publishing

Writing code is just one area where Emacs shines. Therefore, in the future EON may include setups for writing, typesetting and publishing -- e.g. papers, books/ebooks and blogs.
...
- *2.* Put this line into your Emacs init file (=~/.emacs.d/init.el= or =~/.emacs=); ideally quite early after the =(custom-set-faces ...)= form:
#+begin_src emacs-lisp
(load-file "~/.emacs.ontop/eon-init.el")
#+end_src

- *3.* Restart Emacs. Nothing will be installed at this point.
...
- Don't be invasive, nor patronizing: provide add-ons rather than replacements. In the large: EON tries to integrate into your existing Emacs config rather than aiming to replace it. And in the small: global variables are often used across packages. Simple example: Don't "setq" these variables, but do "add-to-list" instead (this negligence appears very often in instructions for how to setup packages).
...
- Rely only on Emacs features enabled by the GNU Emacs distribution's default build options as a smallest common denominator.
