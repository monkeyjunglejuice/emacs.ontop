...
- /If Tree Sitter is installed on your machine, Emacs ONBOARD will download, build and install required grammars automatically./

It is supposed to be fully modular with strictly separated concerns. There are 4 stages:

0. If you disable all modules, you have bare bones, stock Emacs.

1. With just the =eon= module enabled but nothing else, only built-in Emacs packages are set up and configured, without any 3rd-party packages.

2. With =eon= and =eon-base= modules enabled, you'll get extended comfort provided by some 3rd-pard packages.

3. Everything else builds on top of the stages 0, 1 and 2: AI/LLM integration, programming languages, etc (have a look at =eon-setup-modules= for the full list).
...
*eon-lang-webdev*: Just basic HTML, CSS and JavaScript editing and linting (linters have to be installed separately). Highly under-developed, since I don't do web stuff usually.
...
- *3.* Do =M-x eon-user-setup= or =<leader> x C-u=

- *4.* Visit your user directory via =<leader> x u= and uncomment your desired modules in =eon-setup-modules.el=, then restart Emacs.
...
- Don't be invasive: provide add-ons rather than replacements. In the large: Emacs ONTOP tries to integrate into your existing Emacs config rather than aiming to replace it. And in the small: global variables are often used across packages. Be careful not to `setq' these variables, but do `add-to-list' instead (this negligence appears very often in instructions for how to setup packages).
...
- Portable: Emacs ONTOP Should work on recent Linux/Unix, Windows and MacOS systems; Compatibility with Emacs versions from what's in Debian stable upwards (Debian -- as a fairly conservative GNU/Linux distro -- seems a good measure of things).
...
I've used Doom Emacs for roughly 2 years, because I was seeking a way out of the yak-shaving treadmill. However, I realized that my Doom Emacs config was mostly fighting opinions, and I would be more satisfied with a clean slate and bending everything to MY opinions (har har). So I went back to my own Emacs config and made it publicly accessible.
| Doom Emacs                                                                      | Eon                                                  |
|---------------------------------------------------------------------------------+------------------------------------------------------|
| - Thousands of users                                                            | - No users                                           |
| - Superficially modular, but tightly coupled                                    | - Fully modular, decoupled                           |
| - Throw away your own Emacs config                                              | - Eon is provided as a library                       |
| - Mature and covers a plethora of use-cases                                     | - Unstable and narrow focus, untested                |
| - "Fast" startup (masked), lags often at runtime due to lazy-loading everything | - Slower startup, snappy at runtime                  |
| - Strongly opinionated                                                          | - Provides options, bend it your way                 |
| - Focus on VIM-keybindings (Evil), otherwise under-developed                    | - Agnostic: God-mode, Evil, stock Emacs, Meow, Helix |
...
| - Uses Straigth.el package manager                                              | - Uses built-in Package.el, others planned           |
...
| - Relies on shell script helpers & command line                                 | - Pure Emacs Lisp                                    |
...
| Throw away your own Emacs config                     | Make it your own                                   |
