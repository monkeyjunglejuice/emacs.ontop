...
2. With just the =eon= module enabled but nothing else, only built-in Emacs packages are set up and configured -- without any 3rd-party dependencies. This is also how your fresh installation starts out.

3. With both =eon= and =eon-base= modules enabled, you'll get extended comfort provided by some 3rd-pard packages.
...
Programming language support is rather limited at that time; it covers merely languages I personally like & use -- Lisp, functional, interactive. So there's no support for Python, Rust, JS or Go right now. However, if you'd like to use any of the languages in Emacs that are listed below, give it a go -- and you can integrate your language config easily as a user module.
...
** Emacs for Erlang ★★☆☆☆
*eon-lang-erlang*: Basic Erlang setup with ELP (Whatsapp's language server), Erlang shell, Tree-sitter and Org-mode babel for notebook-style literate programming. 2 stars only because docs lookup doesn't work yet; I guess that requires some wrangling with man pages. Doesn't include things like Edts or Distel, as they rely on dependencies at the UI level. Packages come from Melpa right now, not using the Emacs packages distributed with the Erlang/OTP installation. In the future, there may be a choice which package source to use.
...
** Emacs for Julia ★★★★☆
*eon-lang-julia*: The Julia setup is based on the Julia language server, [[https://github.com/gcv/julia-snail][Snail]] and Tree-sitter. The Snail REPL runs within Vterm, the fastest terminal emulator available for Emacs. Enjoy notebook-style literate programming in Org-mode.
...
** Emacs for Unison ★★☆☆☆
*eon-lang-unison*: Includes the treesitter-enabled Unison mode and LSP config for Eglot.
...
Writing code is just one area where Emacs shines. Therefore, in the future EON may include setups for writing, typesetting and publishing -- e.g. papers, books/ebooks or a (static) blog.
...
- *3.* Restart Emacs. Nothing will be installed at this point, only built-in Emacs packages will be configured.
...
* Get comfortable
Check out the module files in order to learn about the commands and key bindings -- all documentation lives there. If you want to change settings, re-define these settings within your =init.el= or =eon-user.el=.
